...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Игнорирование');
	$__finalCompiled .= '
...
<div class="block">
		...
		foreach ($__vars['ignoring'] AS $__vars['user']) {
			...
			if ($__templater->method($__vars['xf']['visitor'], 'isIgnoring', array($__vars['user'], )) OR $__templater->method($__vars['xf']['visitor'], 'canIgnoreUser', array($__vars['user'], ))) {
				$__compilerTemp2 .= '
					...
					'data-sk-ignore' => '+ В игнор',
					'data-sk-unignore' => '- Из игнора',
				), '', array(
				)) . '
						';
			}
	...
	if ($__compilerTemp1) {
		$__finalCompiled .= '
				<div class="block-row">' . 'Вы пока не игнорируете ни одного пользователя.' . '</div>
			';
	}
	$__finalCompiled .= '
		</ol>
