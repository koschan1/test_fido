...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('В ожидании одобрения');
	$__finalCompiled .= '
...
';
	$__vars['sortOrders'] = array('content_date' => 'Дата размещения', );
	$__finalCompiled .= '
...
<div class="block">
	...
	if ($__vars['filters']['content_type']) {
		$__compilerTemp1 .= '
								<li><a href="' . $__templater->func('link', array('approval-queue', null, $__templater->filter($__vars['filters'], array(array('replace', array(array('content_type' => null, ), )),), false), ), true) . '"
									...
									<span class="filterBar-filterToggle-label">' . 'Тип контента' . $__vars['xf']['language']['label_separator'] . '</span>
	...
	if ($__vars['filters']['order'] AND $__vars['sortOrders'][$__vars['filters']['order']]) {
		...
		if ($__vars['filters']['direction'] == 'asc') {
			$__compilerTemp1 .= 'По возрастанию';
		} else {
			$__compilerTemp1 .= 'По убыванию';
		}
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Нет элементов, соответствующих Вашему фильтру.' . '</div>
';
	} else {
		$__finalCompiled .= '
	<div class="blockMessage">' . 'В настоящее время нет контента, ожидающего одобрения.' . '</div>
';
	}
