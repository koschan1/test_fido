...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if (!$__templater->test($__vars['previousChanges'], 'empty', array())) {
				...
				if ($__vars['change']['change_reason']) {
					$__compilerTemp3 .= '
							<a role="button"
									...
									title="' . 'Причина изменения' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['change']['change_reason']) . '">' . trim('
					...
					if ($__vars['change']['reject_reason']) {
						$__compilerTemp4 .= '
								<a role="button"
									...
									data-xf-init="tooltip"
									data-trigger="hover focus click"
									title="' . 'Причина' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['change']['reject_reason']) . '">' . trim('
									' . $__templater->fontAwesome('fa-info-circle', array(
						)) . '
								') . '</a>
							';
		...
		$__compilerTemp1 .= $__templater->dataList('
			...
			'colspan' => '4',
			'_type' => 'cell',
			'html' => 'Журнал изменений имени пользователя',
		))) . '
			' . $__templater->dataRow(array(
			'rowtype' => 'header',
		), array(array(
	...
	<div class="pairWrapper pairWrapper--spaced">
		<dl class="pairs pairs--columns pairs--fixedSmall">
			<dt>' . 'Текущее имя пользователя' . '</dt>
			<dd>' . $__templater->escape($__vars['content']['old_username']) . '</dd>
		</dl>

		<dl class="pairs pairs--columns pairs--fixedSmall">
			<dt>' . 'Новое имя пользователя' . '</dt>
...
' . $__templater->callMacro('approval_queue_macros', 'item_message_type', array(
		...
		'typePhraseHtml' => 'Изменение имени пользователя',
