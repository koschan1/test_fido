...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('API-ключи');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить API-ключ', array(
...
';
	...
	<div class="blockMessage blockMessage--important blockMessage--iconic">
		' . 'Создан или обновлён API-ключ "' . $__templater->escape($__vars['newKey']['title']) . '". Он выглядит следующим образом:' . '
		...
		if ($__templater->isTraversable($__vars['apiKeys'])) {
			foreach ($__vars['apiKeys'] AS $__vars['apiKey']) {
				...
				$__compilerTemp1 .= $__templater->dataRow(array(
					...
					'submit' => 'true',
					'tooltip' => 'Включить/отключить \'' . $__vars['apiKey']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
