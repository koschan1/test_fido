					...
					'html' => '
									...
									<div class="menu" data-menu="menu" aria-hidden="true">
										<div class="menu-content">
											...
											<a href="' . $__templater->func('link', array($__vars['linkPrefix'] . '/add', null, array($__vars['parentIdKey'] => $__vars['category'][$__vars['parentIdKey']], ), ), true) . '" class="menu-linkRow">' . 'Одноуровневый элемент' . '</a>
											<a href="' . $__templater->func('link', array($__vars['linkPrefix'] . '/add', null, array($__vars['parentIdKey'] => $__vars['category'][$__vars['idKey']], ), ), true) . '" class="menu-linkRow">' . 'Дочерний элемент' . '</a>
										</div>
									</div>
								',
				);
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<li class="nestable-item" data-id="' . $__templater->escape($__vars['child']['id']) . '">
		<div class="nestable-handle" aria-label="' . $__templater->filter('Перетаскивание', array(array('for_attr', array()),), true) . '">' . $__templater->fontAwesome('fa-bars', array(
	)) . '</div>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['category'], 'hasChildren', array())) {
		$__compilerTemp1 .= '
			...
			'value' => 'move',
			'selected' => true,
			'label' => 'Переместить дочерние элементы этой категории в родительскую',
			'_type' => 'option',
		),
		array(
			'value' => 'delete',
			'label' => 'Удалить дочерние узлы этой категории' . ' ',
			'_type' => 'option',
		)), array(
		)) . '
				';
