<?php
// FROM HASH: 9fbc5b2199952399149492643ca5ed07
return array(
'code' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
	if ($__templater->method($__vars['memberStat'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Добавить пользовательскую статистику');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Редактировать статистику' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['memberStat']['title']));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '

';
	if ($__templater->method($__vars['memberStat'], 'isUpdate', array()) AND $__templater->method($__vars['memberStat'], 'canEdit', array())) {
		$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('', array(
			'href' => $__templater->func('link', array('member-stats/delete', $__vars['memberStat'], ), false),
			'icon' => 'delete',
			'overlay' => 'true',
		), '', array(
		)) . '
');
	}
	$__finalCompiled .= '

';
	if ((!$__templater->method($__vars['memberStat'], 'canEdit', array()))) {
		$__finalCompiled .= '
	<div class="blockMessage blockMessage--important blockMessage--iconic">
		' . 'В этом элементе можно отредактировать только некоторые поля.' . '
	</div>
';
	}
	$__finalCompiled .= '

';
	$__compilerTemp1 = $__vars;
	$__compilerTemp1['noSpecificUser'] = true;
	$__compilerTemp1['readOnly'] = (!$__templater->method($__vars['memberStat'], 'canEdit', array()));
	$__compilerTemp2 = '';
	if ($__templater->method($__vars['memberStat'], 'canEdit', array())) {
		$__compilerTemp2 .= '
				';
		$__compilerTemp3 = $__templater->mergeChoiceOptions(array(), $__vars['sortOrders']);
		$__compilerTemp2 .= $__templater->formRow('
					<div class="inputPair">
						' . $__templater->formSelect(array(
			'name' => 'sort_order',
			'value' => $__vars['memberStat']['sort_order'],
		), $__compilerTemp3) . '

					' . $__templater->formSelect(array(
			'name' => 'sort_direction',
			'value' => $__vars['memberStat']['sort_direction'],
		), array(array(
			'value' => 'desc',
			'label' => 'По убыванию',
			'_type' => 'option',
		),
		array(
			'value' => 'asc',
			'label' => 'По возрастанию',
			'_type' => 'option',
		))) . '
					</div>
				', array(
			'label' => 'Сортировка',
			'rowtype' => 'input',
		)) . '
			';
	} else {
		$__compilerTemp2 .= '
				';
		$__compilerTemp4 = '';
		if ($__vars['memberStat']['sort_direction'] == 'desc') {
			$__compilerTemp4 .= '
						' . 'По убыванию' . '
					';
		} else {
			$__compilerTemp4 .= '
						' . 'По возрастанию' . '
					';
		}
		$__compilerTemp2 .= $__templater->formRow('
					' . $__templater->escape($__vars['sortOrders'][$__vars['memberStat']['sort_order']]) . '
					&middot;
					' . $__compilerTemp4 . '

					' . $__templater->formHiddenVal('sort_order', $__vars['memberStat']['sort_order'], array(
		)) . '
					' . $__templater->formHiddenVal('sort_direction', $__vars['memberStat']['sort_direction'], array(
		)) . '
				', array(
			'label' => 'Сортировка',
		)) . '
			';
	}
	$__compilerTemp5 = array(array(
		'value' => '',
		'label' => $__vars['xf']['language']['parenthesis_open'] . 'Нет' . $__vars['xf']['language']['parenthesis_close'],
		'_type' => 'option',
	));
	if ($__templater->isTraversable($__vars['permissionsData']['interfaceGroups'])) {
		foreach ($__vars['permissionsData']['interfaceGroups'] AS $__vars['interfaceGroupId'] => $__vars['interfaceGroup']) {
			$__compilerTemp5[] = array(
				'label' => $__vars['interfaceGroup']['title'],
				'_type' => 'optgroup',
				'options' => array(),
			);
			end($__compilerTemp5); $__compilerTemp6 = key($__compilerTemp5);
			if ($__templater->isTraversable($__vars['permissionsData']['permissionsGrouped'][$__vars['interfaceGroupId']])) {
				foreach ($__vars['permissionsData']['permissionsGrouped'][$__vars['interfaceGroupId']] AS $__vars['permission']) {
					$__compilerTemp5[$__compilerTemp6]['options'][] = array(
						'value' => $__vars['permission']['permission_group_id'] . '-' . $__vars['permission']['permission_id'],
						'label' => '
								' . $__templater->escape($__vars['permission']['title']) . '
							',
						'_type' => 'option',
					);
				}
			}
		}
	}
	$__finalCompiled .= $__templater->form('
	<div class="block-container">
		<div class="block-body">
			' . $__templater->formTextBoxRow(array(
		'name' => 'title',
		'value' => ($__vars['memberStat']['member_stat_id'] ? $__vars['memberStat']['MasterTitle']['phrase_text'] : ''),
		'readonly' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
	), array(
		'label' => 'Заголовок',
	)) . '

			' . $__templater->formTextBoxRow(array(
		'name' => 'member_stat_key',
		'value' => $__vars['memberStat']['member_stat_key'],
		'maxlength' => $__templater->func('max_length', array($__vars['memberStat'], 'member_stat_key', ), false),
		'readonly' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
	), array(
		'label' => 'Ключ статистики',
	)) . '

			<hr class="formRowSep" />

			' . $__templater->formRadioRow(array(
		'name' => 'user_criteria',
		'readonly' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
	), array(array(
		'value' => 'all',
		'selected' => !$__vars['memberStat']['criteria'],
		'label' => 'Все действующие пользователи',
		'_type' => 'option',
	),
	array(
		'value' => 'searcher',
		'selected' => $__vars['memberStat']['criteria'],
		'label' => 'Критерии поиска',
		'data-xf-init' => 'disabler',
		'data-container' => '.js-hiderContainer',
		'data-hide' => 'yes',
		'_type' => 'option',
	)), array(
		'label' => 'Критерии пользователя',
	)) . '

			<div class="js-hiderContainer">
				<hr class="formRowSep" />

				' . $__templater->includeTemplate('helper_user_search_criteria', $__compilerTemp1) . '

				<hr class="formRowSep" />
			</div>

			' . $__compilerTemp2 . '

			' . $__templater->formRow('

				' . $__templater->callMacro('helper_callback_fields', 'callback_fields', array(
		'data' => $__vars['memberStat'],
		'readOnly' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
	), $__vars) . '
			', array(
		'rowtype' => 'input',
		'label' => 'PHP-обработчик',
		'explain' => 'Вы можете дополнительно настроить здесь PHP-обработчик, чтобы манипулировать поиском с дополнительными критериями или переопределить существующие критерии.<br />
<br />
Аргументы обработчика:
<ol>
	<li>
		<code>\\XF\\Entity\\MemberStat $memberStat</code>
		<br />
		Элемент статистики этого пользователя.
	</li>
	<li>
		<code>\\XF\\Finder\\User $finder</code>
		<br />
		Поиск пользователя настроен с параметрами критериев/порядка, которые уже указаны выше.
	</li>
</ol>
Обработчик должен либо возвращать массив, либо не возвращать ничего. Если ничего не будет возвращено, то результаты будут отформатированы автоматически. Пример массива:<br />
<br />
<code>return [$user->user_id => \\XF::language()->numberFormat($value)];</code>',
	)) . '

			<hr class="formRowSep" />

			' . $__templater->formSelectRow(array(
		'name' => 'permission_limit',
		'value' => $__vars['memberStat']['permission_limit_'],
		'disabled' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
		'readonly' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
	), $__compilerTemp5, array(
		'label' => 'Ограничение прав',
		'explain' => 'Если здесь выбрано право доступа, то эта статистика будет доступна только пользователям с таким правом доступа.',
	)) . '

			' . $__templater->formRow('

				' . $__templater->callMacro('helper_callback_fields', 'callback_fields', array(
		'data' => $__vars['memberStat'],
		'className' => 'visibility_class',
		'methodName' => 'visibility_method',
		'readOnly' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
	), $__vars) . '
			', array(
		'rowtype' => 'input',
		'label' => 'Обработчик видимости',
		'explain' => 'Вы можете использовать здесь PHP-обработчик, чтобы определить, должен ли отображаться статус пользователя.<br />
<br />
Аргументы обработчика:
<ol>
	<li>
		<code>\\XF\\Entity\\MemberStat $memberStat</code>
		<br />
		Это статистика пользователя.
	</li>
</ol>
Обработчик должен вернуть <code>true</code> или <code>false</code>, где <code>false</code> не позволит отображать статистику пользователя.',
	)) . '

			' . $__templater->formCheckBoxRow(array(
	), array(array(
		'name' => 'show_value',
		'selected' => $__vars['memberStat']['show_value'],
		'readonly' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
		'label' => 'Показывать значение',
		'hint' => 'Если включено и имеется, то значение будет показано для каждого пользователя.',
		'_type' => 'option',
	),
	array(
		'name' => 'overview_display',
		'selected' => $__vars['memberStat']['overview_display'],
		'label' => 'Показывать на вкладке обзора',
		'hint' => (('Если включено, то первые 5 пользователей из этой статистики будут отображены на вкладке "Обзор".' . ' ') . (($__vars['xf']['development'] AND $__vars['memberStat']['addon_id']) ? 'Значение этого поля не будет изменено при обновлении плагина.' : '')),
		'_type' => 'option',
	)), array(
		'label' => 'Настройки',
	)) . '

			' . $__templater->formNumberBoxRow(array(
		'name' => 'user_limit',
		'value' => $__vars['memberStat']['user_limit'],
		'min' => '0',
		'step' => '1',
	), array(
		'label' => 'Лимит пользователей',
		'explain' => (($__vars['xf']['development'] AND $__vars['memberStat']['addon_id']) ? 'Значение этого поля не будет изменено при обновлении плагина.' : ''),
	)) . '

			' . $__templater->callMacro('display_order_macros', 'row', array(
		'value' => $__vars['memberStat']['display_order'],
		'explain' => (($__vars['xf']['development'] AND $__vars['memberStat']['addon_id']) ? 'Значение этого поля не будет изменено при обновлении плагина.' : ''),
	), $__vars) . '

			' . $__templater->formCheckBoxRow(array(
		'readonly' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
	), array(array(
		'selected' => $__vars['memberStat']['cache_lifetime'],
		'label' => 'Включить кэширование и кэшировать данные каждые X минут' . $__vars['xf']['language']['label_separator'],
		'_dependent' => array($__templater->formNumberBox(array(
		'name' => 'cache_lifetime',
		'value' => ($__vars['memberStat']['cache_lifetime'] ?: 60),
		'min' => '0',
		'readonly' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
	))),
		'_type' => 'option',
	)), array(
	)) . '

			' . $__templater->formCheckBoxRow(array(
	), array(array(
		'name' => 'active',
		'selected' => $__vars['memberStat']['active'],
		'label' => 'Статистика активна',
		'hint' => (('Если отключено, то статистика не будет отображаться на странице пользователей.' . ' ') . (($__vars['xf']['development'] AND $__vars['memberStat']['addon_id']) ? 'Значение этого поля не будет изменено при обновлении плагина.' : '')),
		'_type' => 'option',
	)), array(
	)) . '

			' . $__templater->callMacro('addon_macros', 'addon_edit', array(
		'addOnId' => $__vars['memberStat']['addon_id'],
	), $__vars) . '
		</div>

		' . $__templater->formSubmitRow(array(
		'icon' => 'save',
		'sticky' => 'true',
	), array(
	)) . '
	</div>
', array(
		'action' => $__templater->func('link', array('member-stats/save', $__vars['memberStat'], ), false),
		'ajax' => 'true',
		'class' => 'block',
	));
	return $__finalCompiled;
}
);