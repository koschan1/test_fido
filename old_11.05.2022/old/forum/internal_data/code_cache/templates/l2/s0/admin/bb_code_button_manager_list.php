...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Менеджер кнопок BB-кодов');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить выпадающее меню', array(
...
<div class="block">
	<div class="block-container">
		<h2 class="block-header">
			' . 'Панели инструментов' . '
			<span class="block-desc">' . 'Полнотекстовый редактор поддерживает всего четыре возможных конфигурации кнопок, которые применяются в зависимости от размера экрана устройства пользователя.' . '</span>
		</h2>
		...
		if ($__templater->isTraversable($__vars['editorDropdowns'])) {
			foreach ($__vars['editorDropdowns'] AS $__vars['editorDropdown']) {
				$__compilerTemp2 .= '
					...
					'submit' => 'true',
					'tooltip' => 'Включить/отключить \'' . $__vars['editorDropdown']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
