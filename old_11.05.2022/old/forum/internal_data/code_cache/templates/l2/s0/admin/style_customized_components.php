...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Изменённые компоненты стиля' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['style']['title']));
	$__finalCompiled .= '
			...
			if ($__templater->isTraversable($__vars['templates'])) {
				foreach ($__vars['templates'] AS $__vars['template']) {
					$__compilerTemp1 .= '
						...
						'hint' => ($__vars['template']['version_string'] ? 'Изменено в ' . $__templater->escape($__vars['template']['version_string']) . '.' : ''),
						'_type' => 'main',
						'html' => '',
					),
					array(
	...
	<div class="block">
		...
		<div class="block-container">
			<div class="block-body block-row">
				' . 'Этот стиль не содержит изменённых шаблонов или настроек.' . '
			</div>
		</div>
	</div>
';
