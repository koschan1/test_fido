...
';
	if ($__vars['testSuccess'] AND $__vars['xf']['visitor']['email']) {
		$__finalCompiled .= '
	<div class="blockMessage blockMessage--success blockMessage--iconic">' . 'Тестовый email со сводкой активности был успешно отправлен на ' . $__templater->escape($__vars['xf']['visitor']['email']) . '.' . '</div>
';
	}
	$__finalCompiled .= '
...
';
	...
	if (!$__templater->test($__vars['sections'], 'empty', array()) AND $__vars['xf']['visitor']['email']) {
		$__compilerTemp1 .= '
		' . $__templater->button('Отправить тестовое письмо', array(
