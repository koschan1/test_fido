...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['style'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Добавить стиль');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Редактировать стиль' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['style']['title']));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
';
		...
		foreach ($__vars['style']['effective_assets'] AS $__vars['key'] => $__vars['path']) {
			...
			if ($__templater->method($__vars['style'], 'isAssetCustom', array($__vars['key'], ))) {
				...
				$__compilerTemp3 .= '
					...
					'title' => ($__templater->method($__vars['style'], 'isAssetInherited', array($__vars['key'], )) ? 'Изменить в этом стиле' : 'Вернуть родительское значение'),
				), '', array(
				)) . '
							';
			}
	...
	<div class="block-container">
		...
		'class' => 'button--plain button--iconOnly button--adminStyleAsset is-disabled',
			...
			', array(
		'label' => 'Расположение ресурсов',
		'explain' => 'Пути к расположению ресурсов, подключенных к этому стилю, относительно каталога XenForo. Пути к ресурсам можно использовать в настройках стиля с помощью ключа <code>%ASSET:key%</code> или в шаблонах с помощью функции <code>asset()</code>.',
		'rowtype' => 'input',
