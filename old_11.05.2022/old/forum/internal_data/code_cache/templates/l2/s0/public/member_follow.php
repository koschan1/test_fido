...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped(($__vars['isFollowing'] ? 'Отписаться от ' . $__templater->escape($__vars['user']['username']) . '' : 'Подписаться на ' . $__templater->escape($__vars['user']['username']) . ''));
	$__finalCompiled .= '
...
';
	...
	if ($__vars['isFollowing']) {
		$__compilerTemp1 .= '
					' . 'Пожалуйста, подтвердите прекращение подписки на' . $__vars['xf']['language']['label_separator'] . '
				';
	} else {
		$__compilerTemp1 .= '
					' . 'Пожалуйста, подтвердите подписку на' . $__vars['xf']['language']['label_separator'] . '
				';
	}
	...
	<div class="block-container">
		...
		'submit' => ($__vars['isFollowing'] ? 'Отписаться' : 'Подписаться'),
	), array(
		'rowtype' => 'simple',
	)) . '
	</div>
