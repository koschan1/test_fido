...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Реакции');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	<div class="buttonGroup">
		' . $__templater->button('Добавить реакцию', array(
...
,array(
			...
			if ($__templater->method($__vars['reaction'], 'canToggle', array())) {
				$__compilerTemp4[] = array(
					...
					'submit' => 'true',
					'tooltip' => 'Включить/отключить \'' . $__vars['reaction']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				);
			} else {
	...
	if ($__compilerTemp2) {
		$__compilerTemp1 .= '
			...
			'_type' => 'cell',
			'html' => '
							' . 'Пока не добавлено ни одной реакции.' . '
						',
		))) . '
				';
	}
