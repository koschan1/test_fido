...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['command'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Добавить команду');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Редактировать команду' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['command']['command_name']));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
';
	...
	<div class="block-container">
		...
		'explain' => 'Название команды, которая будет использоваться после /',
	)) . '
		...
		'explain' => 'Информация о том, как использовать команду. Это может быть простой текст, HTML или фраза - <b>phrase:my_phrase_title</b>',
	)) . '
		...
		'label' => 'Шаблон параметров команды',
		'explain' => 'Позволяет подключить шаблон, который содержит собственные параметры команды, которые можно использовать в обработчике.',
	)) . '
