...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= $__templater->formCheckBoxRow(array(
		...
		'label' => 'Разрешить голосовать против предложений',
...
';
	...
	', array(
			'label' => 'Префиксы для реализованных',
			'explain' => 'Это префиксы для реализованного предложения. Пользователи могут быстро фильтровать предложения, чтобы увидеть только реализованные. В реализованных предложениях голосование закрыто. Можно выбрать только доступные префиксы.',
			'rowtype' => 'input',
		)) . '
	...
	', array(
			'label' => 'Префиксы для закрытых',
			'explain' => 'Это префиксы для предложения, которое было закрыто (но не реализовано). В закрытых предложениях голосовать нельзя. Можно выбрать только доступные префиксы.',
			'rowtype' => 'input',
		)) . '
	...
	';
		...
		jQuery.extend(XF.phrases, {
			s2_no_results: "' . $__templater->filter('Результатов не найдено.', array(array('escape', array('js', )),), false) . '"
		});
	');
		$__finalCompiled .= '
';
...
';
	...
	' . $__templater->formRow('
		' . $__templater->button('
			' . 'Преобразовать реакции в голоса' . '
		', array(
