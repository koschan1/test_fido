...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Команды');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить команду', array(
		...
		if ($__templater->isTraversable($__vars['commands'])) {
			foreach ($__vars['commands'] AS $__vars['command']) {
				...
				array(
					...
					'submit' => 'true',
					'tooltip' => 'Включить/Выключить команду',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
