...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped(($__vars['group'] ? $__templater->escape($__vars['group']['title']) : 'Свойства без групп'));
	$__finalCompiled .= '
';
	if ($__vars['group']) {
		$__finalCompiled .= '
...
';
	$__templater->breadcrumb($__templater->preEscaped($__templater->escape($__vars['style']['title']) . ' - ' . 'Настройки стиля'), $__templater->func('link', array('styles/style-properties', $__vars['style'], ), false), array(
	));
	$__finalCompiled .= '
	...
	';
		...
		<div class="block-outer">
			...
			'routeParams' => array('group' => $__vars['group']['group_name'], ),
				...
				' . $__templater->button('Группа' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['group']['title']), array(
			...
			'aria-haspopup' => 'true',
				...
				<div class="menu" data-menu="menu" aria-hidden="true">
					<div class="menu-content">
						<h3 class="menu-header">' . 'Группы' . '</h3>
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Эта группа не имеет никаких настроек стиля.' . '</div>
';
	}
	$__finalCompiled .= '
