...
<div class="block"' . $__templater->func('widget_data', array($__vars['widget'], ), true) . '>
	<div class="block-container">
		<h3 class="block-minorHeader">' . ($__templater->escape($__vars['title']) ?: 'Пользователи в чате') . '</h3>
	...
	if ($__vars['options']['search'] AND ($__templater->func('count', array($__vars['users'], ), false) >= $__vars['options']['search'])) {
		...
		$__finalCompiled .= '
				<input type="search" class="input siropuChatFindItem" autocomplete="off" placeholder="' . 'Найти пользователя...' . '" data-xf-init="siropu-chat-find-item">
			';
	}
	$__finalCompiled .= '
	...
	if ($__templater->func('count', array($__vars['users'], ), false)) {
		...
		if ($__templater->isTraversable($__vars['users'])) {
			foreach ($__vars['users'] AS $__vars['user']) {
				$__finalCompiled .= '
						<li data-name="' . $__templater->filter($__vars['user']['username'], array(array('for_attr', array()),), true) . '" title="' . $__templater->filter('Последняя активность', array(array('for_attr', array()),), true) . ': ' . $__templater->func('date_time', array($__vars['user']['siropu_chat_last_activity'], ), true) . '">
							';
		...
		$__finalCompiled .= '
					<li>' . 'Никто не разговаривает в данный момент.' . '</li>
				';
	}
	$__finalCompiled .= '
			</ul>
	...
	if ($__templater->func('count', array($__vars['users'], ), false) >= 10) {
		$__finalCompiled .= '
			<div class="block-footer">
				<span class="block-footer-counter">' . 'Всего' . ': ' . $__templater->func('count', array($__vars['users'], ), true) . '</span>
			</div>
		';
	}
	$__finalCompiled .= '
