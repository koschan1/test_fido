		...
		if ($__vars['showFooter']) {
			$__finalCompiled .= '
				<div class="memberOverviewBlock-seeMore">
					<a href="' . $__templater->func('link', array('members', null, array('key' => $__vars['memberStat']['member_stat_key'], ), ), true) . '">' . 'Посмотреть ещё' . $__vars['xf']['language']['ellipsis'] . '</a>
				</div>
			';
		}
		$__finalCompiled .= '
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped(($__vars['active'] ? $__templater->escape($__vars['active']['title']) : 'Выдающиеся пользователи'));
	$__finalCompiled .= '
...
';
	if ($__vars['userNotFound']) {
		$__finalCompiled .= '
	<div class="blockMessage blockMessage--error blockMessage--iconic">' . 'Указанный пользователь не найден. Пожалуйста, введите другое имя.' . '</div>
';
	}
	$__finalCompiled .= '
...
';
	if ($__templater->test($__vars['memberStats'], 'empty', array())) {
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Нет выдающихся пользователей для отображения.' . '</div>
';
	} else {
		$__finalCompiled .= '
	<section class="block">
		...
		if ($__vars['active']) {
			...
			if ($__compilerTemp2) {
				$__finalCompiled .= '
						<li class="block-row">' . 'Нет пользователей, соответствующих указанным критериям.' . '</li>
					';
			}
			$__finalCompiled .= '
				</ol>
