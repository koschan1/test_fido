...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Архив');
	$__finalCompiled .= '
...
';
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canSearchSiropuChatArchive', array())) {
		...
		$__compilerTemp1 .= $__templater->form('
			...
			'placeholder' => 'Есть ключевые слова',
		)) . '
								<span class="inputGroup-splitter"></span>
								' . $__templater->formTextBox(array(
			'name' => 'users',
	...
	if (!$__templater->test($__vars['messages'], 'empty', array())) {
		...
		$__compilerTemp6 .= '
			...
			'value' => 'newest',
			'label' => 'Сначала, новые сообщения',
			'_type' => 'option',
		),
		array(
			'value' => 'oldest',
			'label' => 'Сначала старые сообщения',
			'_type' => 'option',
		),
		array(
			'value' => 'likes',
			'label' => 'Сначала, сообщения в которых больше всего симпатий',
			'_type' => 'option',
		))) . '
							   <span class="inputGroup-splitter"></span>
