...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Ваша лента новостей');
	$__finalCompiled .= '
...
<div class="block">
	...
	if (!$__templater->test($__vars['newsFeedItems'], 'empty', array())) {
		...
		$__finalCompiled .= '
			<div class="block-body js-newsFeedTarget">
				<div class="block-row">
					' . 'Ваша лента новостей пуста.' . '
					<a href="' . $__templater->func('link', array('account/following', ), true) . '">' . 'Подписывайтесь на пользователей для её наполнения.' . '</a>
				</div>
			</div>
		';
	}
