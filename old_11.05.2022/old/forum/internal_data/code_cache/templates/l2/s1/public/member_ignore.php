...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped(($__vars['isIgnoring'] ? 'Перестать игнорировать ' . $__templater->escape($__vars['user']['username']) . '' : 'Игнорировать ' . $__templater->escape($__vars['user']['username']) . ''));
	$__finalCompiled .= '
...
';
	...
	if ($__vars['isIgnoring']) {
		$__compilerTemp1 .= '
					' . 'Пожалуйста, подтвердите прекращение игнорирования' . $__vars['xf']['language']['label_separator'] . '
				';
	} else {
		$__compilerTemp1 .= '
					' . 'Пожалуйста, подтвердите игнорирование' . $__vars['xf']['language']['label_separator'] . '
				';
	}
	...
	<div class="block-container">
		...
		'submit' => ($__vars['isIgnoring'] ? '- Из игнора' : '+ В игнор'),
	), array(
		'rowtype' => 'simple',
	)) . '
	</div>
