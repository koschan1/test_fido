...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Трофеи, присуждённые ' . $__templater->escape($__vars['user']['username']) . '');
	$__finalCompiled .= '
...
<div class="block">
	...
	if (!$__templater->test($__vars['trophies'], 'empty', array())) {
		...
		$__finalCompiled .= '
			<div class="block-body block-row">' . '' . $__templater->escape($__vars['user']['username']) . ' ещё не награждён(а) ни одним трофеем.' . '</div>
		';
	}
	$__finalCompiled .= '
		<div class="block-footer block-footer--split">
			<span class="block-footer-counter">' . 'Всего баллов' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('number', array($__vars['user']['trophy_points'], ), true) . '</span>
