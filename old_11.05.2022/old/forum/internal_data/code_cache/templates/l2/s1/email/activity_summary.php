...
<mail:subject>' . $__templater->escape($__vars['xf']['options']['boardTitle']) . ' - ' . 'Посмотрите, что Вы пропустили' . '</mail:subject>

';
	if ($__vars['displayValues']) {
		$__finalCompiled .= '
	' . $__templater->callMacro('activity_summary_macros', 'outer_header', array(
			'title' => 'Краткая сводка того, что Вы пропустили в последнее время',
		), $__vars) . '
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" class="container container--outer">
	<tr>
		<td align="center" valign="top">
			<p class="minorText">' . 'Если Вы больше не хотите получать такие письма, Вы можете <a href="' . $__templater->func('link', array('canonical:email-stop/activity-summary', $__vars['xf']['toUser'], ), true) . '">отключить отправку сводки активности на email</a> или <a href="' . $__templater->func('link', array('canonical:email-stop/all', $__vars['xf']['toUser'], ), true) . '">отключить отправку всех email</a>.' . '</p>
		</td>
	</tr>
</table>';
