...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Полученные реакции');
	$__finalCompiled .= '
			...
			if ($__templater->isTraversable($__vars['tabSummary'])) {
				foreach ($__vars['tabSummary'] AS $__vars['reactionId'] => $__vars['total']) {
					...
					if ($__vars['reactionId']) {
						...
						$__finalCompiled .= '
									<bdi>' . 'Все' . '</bdi> ' . $__templater->filter($__vars['total'], array(array('number', array()),array('parens', array()),), true) . '
								';
					}
					$__finalCompiled .= '
							</a>
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'К сожалению, на Ваш контент ещё никто не отреагировал.' . '</div>
';
	}
	$__finalCompiled .= '
