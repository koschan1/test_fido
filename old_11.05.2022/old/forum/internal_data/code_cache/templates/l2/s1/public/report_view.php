	...
	if ($__vars['report']['link']) {
		$__finalCompiled .= '
			<div class="block-footer">
				<span class="block-footer-controls">' . $__templater->button('Посмотреть в контексте', array(
...
<div class="block">
	<div class="block-container">
		...
		<h2 class="block-tabHeader tabs" data-xf-init="tabs" role="tablist">
			...
			<a class="tabs-tab" role="tab" tabindex="0" aria-controls="' . $__templater->func('unique_id', array('reportReassign', ), true) . '">' . 'Поручить обработку' . '</a>
		</h2>
	...
	if ($__vars['assignedToOther']) {
		$__compilerTemp2 .= '
							' . $__templater->formRow('
								' . 'Обработка этой жалобы в настоящее время поручена ' . $__templater->escape($__vars['report']['AssignedUser']['username']) . '.' . '
								<div>
									' . $__templater->button('
										' . 'Обработать жалобу' . '
									', array(
	...
	if (!$__templater->method($__vars['report'], 'isClosed', array())) {
		...
		array(
			'value' => 'assigned',
			'label' => (($__vars['report']['report_state'] == 'assigned') ? 'Не обрабатывается' : 'Обрабатывается'),
			'_type' => 'option',
		),
		array(
			'value' => 'resolved',
