...
if(isAjax() == true){
   ...
   update("UPDATE uni_settings SET value=? WHERE name=?", array(intval($_POST["pwa_status"]),'pwa_status'));
    ...
    $manifest["icons"][] = [ "src" => $config["urlPath"] . "/templates/icons_pwa/icon-192x192.png", "sizes" => "192x192", "type" => "image/png", "purpose" => "any maskable" ];
    $manifest["icons"][] = [ "src" => $config["urlPath"] . "/templates/icons_pwa/icon-256x256.png", "sizes" => "256x256", "type" => "image/png" ];
    $manifest["icons"][] = [ "src" => $config["urlPath"] . "/templates/icons_pwa/icon-384x384.png", "sizes" => "384x384", "type" => "image/png" ];
    $manifest["icons"][] = [ "src" => $config["urlPath"] . "/templates/icons_pwa/icon-512x512.png", "sizes" => "512x512", "type" => "image/png" ];
    $manifest["screenshots"][] = [ "src" => $config["urlPath"] . "/templates/icons_pwa/icon-512x512.png", "sizes" => "512x512", "type" => "image/png" ];

    if(!empty($_FILES['pwa_icon']['name'])){

        $path = $config["basePath"] . "/templates/icons_pwa/";
        $max_file_size = 1;
        ...
        if (in_array($ext, $extensions))
        {
                ...
                {                    
                    ...
                    resize($path, $config["basePath"] . "/templates/icons_pwa/icon-384x384.png" , 384, 384, 100);
                    resize($path, $config["basePath"] . "/templates/icons_pwa/icon-256x256.png" , 256, 256, 100);
                    resize($path, $config["basePath"] . "/templates/icons_pwa/icon-192x192.png" , 192, 192, 100);
                }
