...
if(isAjax() == true){
    ...
    if($secure){
	    ...
	    if($status == 0){
	        ...
	        $Ads->addSecurePayments( ["id_order"=>$secure["secure_id_order"], "amount"=>$secure["secure_price"], "card"=>$user_seller["clients_bank_card"], "id_user"=>$secure["secure_id_user_seller"], "status_pay"=>0, "status"=>1, "amount_percent" => $Ads->secureTotalAmountPercent($secure["secure_price"])] );

	        update("update uni_secure set secure_status=? where secure_id=?", array(3,$id));

	    }elseif($status == 1){

	        $Ads->addSecurePayments( ["id_order"=>$secure["secure_id_order"], "amount"=>$secure["secure_price"], "card"=>$user_buyer["clients_bank_card"], "id_user"=>$secure["secure_id_user_buyer"], "status_pay"=>0, "status"=>2, "amount_percent" => $Ads->secureTotalAmountPercent($secure["secure_price"], false)] );
