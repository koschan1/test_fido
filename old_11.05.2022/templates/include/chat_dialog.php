	   ...
	   foreach ($list as $date => $array) {
		   ...
		   foreach ($array as $key => $value) {
                 ...
                 ?>
					 ...
					 <div class="chat-dialog-content-action" >
					 	<div>
					 	<img src="<?= $settings["path_other"]; ?>/comment.png" />
					 	</div>

					 	<?php echo $ULang->t($value["chat_messages_text"]); ?>
					 	<br><br>
					 	<div class="view-list-status mb10 btn-sm">
					 		<?php
						 		preg_match('#\<i>(.*?)\</i>#', $value["chat_messages_text"], $match);
								$id = trim($match[1], '"');
					 		 ?>
            	<span><a class="text-white" href="<?= $Ads->alias($getAd); ?>?action=reply&id=<?= md5($id) ?>"><?php echo $ULang->t("Ответить"); ?></a></span>
            </div> 
					 </div>
