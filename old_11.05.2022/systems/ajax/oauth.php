...
if($_GET["network"] == "vk"){
...
}elseif($_GET["network"] == "tg"){
	$user_data = [];

	define('BOT_TOKEN', '5034031038:AAEADTRCuNXLxSjUfHLxaiPgmfyQ3rgR-Hc');
	define('BOT_USERNAME', 'XXXXXXXXXX');

	$auth_data = $_GET;
	$check_hash = $auth_data['hash'];
	unset($auth_data['hash'], $auth_data['network']);
	...
	if (strcmp($hash, $check_hash) !== 0) {
		exit('Data is NOT from Telegram');
	}

	if ((time() - $auth_data['auth_date']) > 86400) {
		exit('Data is outdated');
	}

	$image_name = end(explode("/", $auth_data["photo_url"]));
	file_put_contents(__DIR__.'/../../media/images_users/'.$image_name, file_get_contents($auth_data["photo_url"]));
	...
	$result = $Profile->auth_reg(array( 
		...
		"surname"=>$auth_data["last_name"],
		"activation" => 1,
		"social_link" => $auth_data["username"],
		"avatar" => $image_name,
	));
