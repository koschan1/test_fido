...
class Filters
{
   ...
   function queryFilter($array = array(), $param = array())
   {
      ...
      if (intval($array["filter"]["secure"])) {
         $forming_multi_query["secure"] = "category_board_secure='1' and clients_secure='1' and (ads_price BETWEEN " . round($settings["secure_min_amount_payment"], 2) . " AND " . round($settings["secure_max_amount_payment"], 2) . ")";

         $param_search["query"]["bool"]["filter"][]["term"]["category_board_secure"] = 1;
         $param_search["query"]["bool"]["filter"][]["term"]["clients_secure"] = 1;
         $param_search["query"]["bool"]["filter"][]["range"]["ads_price"] = ["gte" => round($settings["secure_min_amount_payment"], 2), "lte" => round($settings["secure_max_amount_payment"], 2)];
                   ...
                   <div class="filter-items" id-filter="' . $value["ads_filters_id"] . '" main-id-filter="0" data-ids="" >
                        ...
                        <div class="row mb15" >
                          ...
                          <div class="col-lg-7">
                              ...
                              <input type="number" step="any" maxlength="15" data-min="' . intval($getItems[0]["ads_filters_items_value"]) . '" data-max="' . intval($getItems[1]["ads_filters_items_value"]) . '" class="form-control" name="filter[' . $value["ads_filters_id"] . '][]" value="' . $getVariants["value"][$value["ads_filters_id"]][0]["ads_filters_variants_val"] . '" />  
      ...
      if ($getFilters["id_parent"][0]) {
         ...
         foreach ($getFilters["id_parent"][0] as $id_filter => $value) {
            ...
            if ($getItems) {
               ...
               if (file_exists($config["basePath"] . "/templates/include/" . $tpl . ".php")) {
                  require $config["basePath"] . "/templates/include/" . $tpl . ".php";
               } else {
                  require $config["basePath"] . "/templates/include/filters_catalog.php";
               }
            }
         }
      }
         ...
         foreach ($getFilters["id_parent"][$id_filter] as $parent_value) {
            ...
            if ($getItems) {
               ...
               if (file_exists($config["basePath"] . "/templates/include/" . $tpl . ".php")) {
                  ...
                  require $config["basePath"] . "/templates/include/podfilters_catalog.php";
               }
            }
         }
      }
