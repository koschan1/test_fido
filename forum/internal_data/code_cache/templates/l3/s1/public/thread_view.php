...
'thread_action_buttons' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['thread'], 'canChangeType', array()) AND ($__templater->func('count', array($__vars['creatableThreadTypes'], ), false) > 1)) {
		$__compilerTemp2 .= '
												<a href="' . $__templater->func('link', array('threads/change-type', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Изменить тип темы' . '</a>
											';
	}
	$__compilerTemp2 .= '
											';
	...
	if ($__vars['thread']['discussion_state'] == 'deleted') {
		...
		$__compilerTemp1 .= '
						<dd class="blockStatus-message blockStatus-message--moderated">
							' . 'Ожидает одобрения перед размещением.' . '
						</dd>
					';
	}
	$__compilerTemp1 .= '
					...
					<div class="message">
						<div class="message-inner">
							<div class="message-cell">
								' . 'Нет сообщений, соответствующих Вашим фильтрам.' . '
							</div>
						</div>
					</div>
				';
