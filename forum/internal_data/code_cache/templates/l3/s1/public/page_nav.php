...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<div class="menu menu--pageJump" data-menu="menu" aria-hidden="true">
		...
		'data-menu-autofocus' => 'true',
					...
					<span class="inputGroup-text">' . $__templater->button('Выполнить', array(
		'class' => 'js-pageJumpGo',
	), '', array(
	)) . '</span>
				</div>
...
<div class="pageNav ' . ($__vars['hasSkipStart'] ? 'pageNav--skipStart' : '') . ' ' . ($__vars['hasSkipEnd'] ? 'pageNav--skipEnd' : '') . '">
	...
	if ($__vars['prev']) {
		$__finalCompiled .= '
		<a href="' . $__templater->escape($__vars['prev']) . '" class="pageNav-jump pageNav-jump--prev">' . 'Назад' . '</a>
	';
	}
	$__finalCompiled .= '
		...
		if ($__vars['startInner'] == 3) {
			...
			$__finalCompiled .= '
				<li class="pageNav-page pageNav-page--skip pageNav-page--skipStart">
					<a data-xf-init="tooltip" title="' . $__templater->filter('Перейти к странице', array(array('for_attr', array()),), true) . '"
						data-xf-click="menu"
	...
	if ($__vars['next']) {
		$__finalCompiled .= '
		<a href="' . $__templater->escape($__vars['next']) . '" class="pageNav-jump pageNav-jump--next">' . 'Вперёд' . '</a>
	';
	}
	$__finalCompiled .= '
</div>
...
<div class="pageNavSimple">
	...
	if ($__vars['current'] > 1) {
		...
		<a href="' . $__templater->func('link', array($__vars['link'], $__vars['data'], $__vars['params'] + array($__vars['pageParam'] => 1, ), $__vars['hash'], ), true) . '"
			...
			data-xf-init="tooltip" title="' . $__templater->filter('Первый', array(array('for_attr', array()),), true) . '">
	...
	<a class="pageNavSimple-el pageNavSimple-el--current"
		data-xf-init="tooltip" title="' . $__templater->filter('Перейти к странице', array(array('for_attr', array()),), true) . '"
		data-xf-click="menu" role="button" tabindex="0" aria-expanded="false" aria-haspopup="true">
		' . '' . $__templater->escape($__vars['current']) . ' из ' . $__templater->escape($__vars['totalPages']) . '' . '
	</a>
	' . $__templater->callMacro(null, 'page_jump_menu', array(
		'page' => $__vars['current'],
		'totalPages' => $__vars['totalPages'],
	...
	if ($__vars['current'] < $__vars['totalPages']) {
		...
		<a href="' . $__templater->func('link', array($__vars['link'], $__vars['data'], $__vars['params'] + array($__vars['pageParam'] => $__vars['totalPages'], ), $__vars['hash'], ), true) . '"
			...
			data-xf-init="tooltip" title="' . $__templater->filter('Последняя', array(array('for_attr', array()),), true) . '">
