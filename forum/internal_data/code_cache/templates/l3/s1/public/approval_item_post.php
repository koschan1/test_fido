<?php
// FROM HASH: 9913f773306175bc51f640c351bb27ab
return array(
'code' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
	$__finalCompiled .= $__templater->callMacro('approval_queue_macros', 'item_message_type', array(
		'content' => $__vars['content'],
		'user' => $__vars['content']['User'],
		'messageHtml' => $__templater->func('bb_code', array($__vars['content']['message'], 'post', $__vars['content'], ), false),
		'typePhraseHtml' => 'Сообщение',
		'spamDetails' => $__vars['spamDetails'],
		'unapprovedItem' => $__vars['unapprovedItem'],
		'handler' => $__vars['handler'],
		'headerPhraseHtml' => 'Размещено в теме <a href="' . $__templater->func('link', array('posts', $__vars['content'], ), true) . '">' . $__templater->escape($__vars['content']['Thread']['title']) . '</a> в разделе <a href="' . $__templater->func('link', array('forums', $__vars['content']['Thread']['Forum'], ), true) . '">' . $__templater->escape($__vars['content']['Thread']['Forum']['title']) . '</a>',
	), $__vars) . '
';
	return $__finalCompiled;
}
);