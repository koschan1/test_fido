...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Сейчас на форуме');
...
<div class="block">
	<div class="block-container">
		<h2 class="block-tabHeader tabs hScroller" data-xf-init="h-scroller">
			<span class="hScroller-scroll">
				' . '
				<a href="' . $__templater->func('link', array('online', ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == '') ? 'is-active' : '') . '">' . 'Все' . '</a>
				<a href="' . $__templater->func('link', array('online', null, array('type' => 'member', ), ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == 'member') ? 'is-active' : '') . '">' . 'Пользователи' . '</a>
				<a href="' . $__templater->func('link', array('online', null, array('type' => 'guest', ), ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == 'guest') ? 'is-active' : '') . '">' . 'Гости' . '</a>
				<a href="' . $__templater->func('link', array('online', null, array('type' => 'robot', ), ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == 'robot') ? 'is-active' : '') . '">' . 'Роботы' . '</a>
				' . '
			</span>
		</h2>
		';
	if (!$__templater->test($__vars['activities'], 'empty', array())) {
			...
			foreach ($__vars['activities'] AS $__vars['activity']) {
				...
				if ($__vars['activity']['User']) {
					...
					$__finalCompiled .= '
									<h3 class="contentRow-header">
										' . 'Робот' . $__vars['xf']['language']['label_separator'] . ' ';
				...
				if ((!$__vars['activity']['User']) OR $__templater->method($__vars['activity']['User'], 'canViewCurrentActivity', array())) {
					...
					if ($__vars['activity']['description']) {
						...
						$__finalCompiled .= '
												<span title="' . $__templater->escape($__vars['activity']['controller_name']) . '::' . $__templater->escape($__vars['activity']['controller_action']) . '">' . 'Смотрит неизвестную страницу' . '</span>
											';
					} else {
						$__finalCompiled .= '
												' . 'Смотрит неизвестную страницу' . '
											';
					}
					$__finalCompiled .= '
											</li>
				...
				if (($__vars['activity']['view_state'] == 'error') AND $__templater->method($__vars['xf']['visitor'], 'canBypassUserPrivacy', array())) {
					$__finalCompiled .= '
						...
						'title' => $__templater->filter('Видит ошибку', array(array('for_attr', array()),), false),
					)) . '
											<span class="u-srOnly">' . 'Видит ошибку' . '</span>
											</li>
										';
				}
				$__finalCompiled .= '
		...
		$__finalCompiled .= '
			<div class="block-row">' . 'Результатов не найдено.' . '</div>
		';
	}
	$__finalCompiled .= '
	</div>
