...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['search']['search_query']) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Результаты поиска по запросу' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['search']['search_query']));
		$__templater->pageParams['pageNumber'] = $__vars['page'];
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageH1'] = $__templater->preEscaped('Результаты поиска по запросу' . $__vars['xf']['language']['label_separator'] . ' <a href="' . $__templater->func('link', array('search', $__vars['search'], array('searchform' => '1', ), ), true) . '"><em>' . $__templater->escape($__vars['search']['search_query']) . '</em></a>');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Результаты поиска');
		$__templater->pageParams['pageNumber'] = $__vars['page'];
		$__finalCompiled .= '
';
	}
...
';
	$__templater->breadcrumb($__templater->preEscaped('Поиск'), $__templater->func('link', array('full:search', ), false), array(
	));
	$__finalCompiled .= '
	...
	if ($__vars['modTypes']) {
		...
		$__finalCompiled .= '
					<div class="buttonGroup-buttonWrapper">
						' . $__templater->button('Выбрать для модерации', array(
			...
			'aria-haspopup' => 'true',
						...
						<div class="menu" data-menu="menu" aria-hidden="true">
							<div class="menu-content">
								<h3 class="menu-header">' . 'Выбрать для модерации' . '</h3>
								';
		...
		if ($__vars['activeModType']) {
			$__finalCompiled .= '
									...
									<a href="' . $__templater->func('link', array('search', $__vars['search'], array('page' => (($__vars['page'] > 1) ? $__vars['page'] : ''), ), ), true) . '" class="menu-linkRow">' . 'Отключить' . '</a>
								';
		}
		$__finalCompiled .= '
							</div>
		...
		$__finalCompiled .= '
			<div class="block-footer">
				<span class="block-footer-controls">' . $__templater->button('
					' . 'Просмотреть более ранние результаты' . '
				', array(
