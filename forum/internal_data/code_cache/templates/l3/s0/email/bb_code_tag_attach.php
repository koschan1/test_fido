...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<a href="' . $__templater->func('link', array('canonical:attachments', array('attachment_id' => $__vars['id'], ), ), true) . '" target="_blank">' . 'Посмотреть вложение ' . $__templater->escape($__vars['id']) . '' . '</a>
');
	} else if (($__vars['attachment']['is_video'] OR $__vars['attachment']['is_audio']) AND ($__vars['full'] AND $__vars['canView'])) {
		$__finalCompiled .= trim('
	<div class="mediaPlaceholder">' . 'Вставленные медиа' . '</div>
');
	} else if (!$__vars['attachment']['has_thumbnail']) {
		$__finalCompiled .= trim('
	<a href="' . $__templater->func('link', array('canonical:attachments', $__vars['attachment'], array('hash' => $__vars['attachment']['temp_hash'], ), ), true) . '" target="_blank">' . 'Посмотреть вложение ' . $__templater->escape($__vars['attachment']['filename']) . '' . '</a>
');
	} else {
		$__finalCompiled .= trim('
