...
'thread_list_header' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<h2 class="block-tabHeader tabs">
		<div class="hScroller" data-xf-init="h-scroller">
			<span class="hScroller-scroll">
				' . '
				<a href="' . $__templater->func('link', array('forums', $__vars['forum'], $__vars['suggestionTabs']['latest']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['suggestionTabs']['latest']['selected'] ? 'is-active' : '') . '">' . 'Последние обновления' . '</a>
				<a href="' . $__templater->func('link', array('forums', $__vars['forum'], $__vars['suggestionTabs']['popular']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['suggestionTabs']['popular']['selected'] ? 'is-active' : '') . '" rel="nofollow">' . 'Популярное' . '</a>
				<a href="' . $__templater->func('link', array('forums', $__vars['forum'], $__vars['suggestionTabs']['newest']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['suggestionTabs']['newest']['selected'] ? 'is-active' : '') . '" rel="nofollow">' . 'Новые' . '</a>
				';
	if ($__vars['suggestionTabs']['implemented']) {
		$__finalCompiled .= '
					<a href="' . $__templater->func('link', array('forums', $__vars['forum'], $__vars['suggestionTabs']['implemented']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['suggestionTabs']['implemented']['selected'] ? 'is-active' : '') . '" rel="nofollow">' . 'Реализовано' . '</a>
				';
	}
	$__finalCompiled .= '
				';
	if ($__vars['suggestionTabs']['yourSuggestions']) {
		$__finalCompiled .= '
					<a href="' . $__templater->func('link', array('forums', $__vars['forum'], $__vars['suggestionTabs']['yourSuggestions']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['suggestionTabs']['yourSuggestions']['selected'] ? 'is-active' : '') . '" rel="nofollow">' . 'Ваши предложения' . '</a>
				';
	}
	$__finalCompiled .= '
				';
	if ($__vars['suggestionTabs']['yourVotes']) {
		$__finalCompiled .= '
					<a href="' . $__templater->func('link', array('forums', $__vars['forum'], $__vars['suggestionTabs']['yourVotes']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['suggestionTabs']['yourVotes']['selected'] ? 'is-active' : '') . '" rel="nofollow">' . 'Ваши голоса' . '</a>
				';
	}
	$__finalCompiled .= '
				' . '
	...
	if ($__vars['filters']['suggestion_state'] AND (($__vars['filters']['suggestion_state'] != 'implemented') OR (!$__vars['suggestionTabs']['implemented']['selected']))) {
		$__finalCompiled .= '
			<li><a href="' . $__templater->func('link', array('forums', $__vars['forum'], $__templater->filter($__vars['filters'], array(array('replace', array('suggestion_state', null, )),), false), ), true) . '"
				...
				<span class="filterBar-filterToggle-label">' . 'Статус предложения' . ':</span>
				';
		if ($__vars['filters']['suggestion_state'] == 'open') {
			$__finalCompiled .= 'Открыть' . '
				';
		} else if ($__vars['filters']['suggestion_state'] == 'closed') {
			$__finalCompiled .= 'Закрыто' . '
				';
		} else if ($__vars['filters']['suggestion_state'] == 'implemented') {
			$__finalCompiled .= 'Реализовано';
		}
