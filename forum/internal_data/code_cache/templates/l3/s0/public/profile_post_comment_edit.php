...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Редактировать комментарий к сообщению в профиле');
	$__finalCompiled .= '
...
';
	...
	$__finalCompiled .= '
';
	$__templater->breadcrumb($__templater->preEscaped('Сообщение в профиле от ' . $__templater->escape($__vars['profilePost']['username']) . ''), $__templater->func('link', array('profile-posts', $__vars['profilePost'], ), false), array(
	));
	$__finalCompiled .= '
	...
	if ($__vars['quickEdit']) {
		$__compilerTemp3 .= '
			...
			'icon' => 'save',
				...
				' . $__templater->button('Отменить', array(
