...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Оповещения');
	$__finalCompiled .= '
...
<div class="block">
	...
	if (!$__templater->test($__vars['alerts'], 'empty', array())) {
		...
		$__finalCompiled .= '
				<div class="block-row">' . 'У Вас нет новых оповещений.' . '</div>
			';
	} else {
		$__finalCompiled .= '
				<div class="block-row">' . 'Нет оповещения для отображения. Пожалуйста, выберите другую страницу.' . '</div>
			';
	}
	$__finalCompiled .= '
		</div>
