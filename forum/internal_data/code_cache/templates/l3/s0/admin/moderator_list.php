...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Модераторы');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить модератора', array(
...
';
	...
	if (!$__templater->test($__vars['superModerators'], 'empty', array())) {
		...
		<div class="block">
			<div class="block-container">
				<h2 class="block-header">' . 'Главные модераторы' . '</h2>
	...
	if (!$__templater->test($__vars['contentModerators'], 'empty', array())) {
			...
			<div class="block-outer">
				<div class="block-outer-main">
					<a class="button button--link menuTrigger"
						...
						tabindex="0"
						aria-expanded="false"
						aria-haspopup="true">' . 'Фильтровать' . $__vars['xf']['language']['label_separator'] . ' ' . ($__vars['userIdFilter'] ? $__templater->escape($__vars['users'][$__vars['userIdFilter']]['username']) : $__vars['xf']['language']['parenthesis_open'] . 'Все' . $__vars['xf']['language']['parenthesis_close']) . '</a>

					<div class="menu" data-menu="menu" aria-hidden="true">
						<div class="menu-content">
							<h3 class="menu-header">' . 'Действующие модераторы' . '</h3>
							<a href="' . $__templater->func('link', array('moderators', ), true) . '" class="menu-linkRow ' . ((!$__vars['userIdFilter']) ? 'is-selected' : '') . '">' . $__vars['xf']['language']['parenthesis_open'] . 'Все' . $__vars['xf']['language']['parenthesis_close'] . '</a>
							';
		...
		$__compilerTemp1 .= '
			...
			<div class="block-container">
				<h2 class="block-header">' . 'Действующие модераторы' . '</h2>
				<div class="block-body">
					';
		$__compilerTemp3 = '';
		if ($__templater->isTraversable($__vars['users'])) {
				...
				if ($__vars['contentModerators'][$__vars['userId']]) {
					...
					if ($__vars['displayLimit'] AND ($__vars['contentModeratorTotals'][$__vars['userId']] > $__vars['displayLimit'])) {
						$__compilerTemp3 .= '
							...
							'_type' => 'cell',
							'html' => '
												<a href="' . $__templater->func('link', array('moderators', null, array('user_id' => $__vars['userId'], ), ), true) . '" data-xf-click="inserter" data-replace=".js-moderatorInsert-' . $__templater->escape($__vars['userId']) . '" data-animate-replace="false">' . '...и ещё ' . $__templater->filter($__vars['contentModeratorTotals'][$__vars['userId']] - $__vars['displayLimit'], array(array('number', array()),), true) . '.' . '</a>
											',
						))) . '
									';
					}
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'На данный момент нет модераторов. Чтобы создать модератора, используйте ссылку выше.' . '</div>
';
	}
