...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Страницы помощи');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить страницу помощи', array(
		...
		if ($__templater->isTraversable($__vars['pages'])) {
			foreach ($__vars['pages'] AS $__vars['page']) {
				$__compilerTemp1 .= '
					...
					'submit' => 'true',
					'tooltip' => 'Включить/отключить \'' . $__vars['page']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
