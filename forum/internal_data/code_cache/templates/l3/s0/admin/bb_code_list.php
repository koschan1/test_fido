...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Собственные BB-коды');
	$__finalCompiled .= '
...
';
	...
	<div class="buttonGroup">
		' . $__templater->button('Добавить BB-код', array(
			...
			foreach ($__vars['bbCodes'] AS $__vars['bbCode']) {
				...
				if ($__vars['exportView']) {
					$__compilerTemp3[] = array(
						...
						'tooltip' => ($__vars['bbCode']['addon_id'] ? 'BB-коды, принадлежащие этому плагину, не могут быть экспортированы как самостоятельные BB-коды.' : ''),
						'_type' => 'toggle',
						'html' => '',
					);
				}
				...
				if (!$__vars['exportView']) {
					$__compilerTemp3[] = array(
						...
						'submit' => 'true',
						'tooltip' => 'Включить/отключить \'' . (('[' . $__vars['bbCode']['bb_code_id']) . ']') . '\'',
						'_type' => 'toggle',
						'html' => '',
					);
					$__compilerTemp3[] = array(
