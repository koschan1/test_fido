...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Позиции виджета');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить позицию виджета', array(
		...
		if ($__templater->isTraversable($__vars['widgetPositions'])) {
			foreach ($__vars['widgetPositions'] AS $__vars['widgetPosition']) {
				$__compilerTemp1 .= '
					...
					'submit' => 'true',
					'tooltip' => 'Включить/отключить \'' . $__vars['widgetPosition']['position_id'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
