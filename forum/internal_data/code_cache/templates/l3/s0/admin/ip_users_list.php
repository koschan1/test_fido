...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Другие пользователи с IP адресом ' . $__templater->escape($__vars['ipPrintable']) . '');
	$__finalCompiled .= '
	...
	if ($__templater->isTraversable($__vars['ips'])) {
		foreach ($__vars['ips'] AS $__vars['key'] => $__vars['ip']) {
			...
			if ($__vars['ip']['ip_total'] > 1) {
				$__compilerTemp2 .= '
										<li>' . '' . $__templater->filter($__vars['ip']['ips']['0'], array(array('ip', array()),), true) . ' и ещё ' . ($__vars['ip']['ip_total'] - 1) . '' . '</li>
									';
			} else {
				$__compilerTemp2 .= '
