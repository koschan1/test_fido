...
';
	...
	if ((!$__vars['existingType']) OR ($__vars['existingType']['options'] == 'text')) {
		...
		array(
			...
			'label' => 'Требования для соответствия значения',
	...
	if ((!$__vars['existingType']) OR ($__vars['existingType']['options'] == 'choice')) {
		$__compilerTemp5 .= '
			<h3 class="block-formSectionHeader">
				<span class="collapseTrigger collapseTrigger--block" data-xf-click="toggle" data-target="< :up:next">
					<span class="block-formSectionHeader-aligner">' . 'Настройки полей с вариантами ответа' . '</span>
				</span>
			</h3>
			<div class="block-body block-body--collapsible">
				';
				...
				$__compilerTemp6 .= '
							<div class="inputGroup">
								<span class="inputGroup-text dragHandle"
									aria-label="' . $__templater->filter('Перетаскивание', array(array('for_attr', array()),), true) . '"></span>
								' . $__templater->formTextBox(array(
					'name' => 'field_choice[]',
					'value' => $__vars['choice'],
	...
	if ($__vars['hasWrapperTemplate']) {
		$__compilerTemp8 .= '
			...
			'label' => 'HTML-обёртка отображения',
			'explain' => 'Это поле позволяет отформатировать содержащуюся в нём разметку и выбранные значения. Это даёт больше гибкости в настройке, но Вы должны использовать заполнитель <b>{$value}</b> для отображения выбранных значений поля. При желании, Вы можете использовать <b>{$title}</b> - для заголовка поля, <b>{$description}</b> - для описание поля и <b>{$fieldId}</b> - для ID этого поля (<span id="FieldId">' . $__templater->escape($__vars['field']['field_id']) . '</span>).',
		)) . '
			';
	}
