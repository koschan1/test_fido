...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Задания планировщика');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить задание планировщика', array(
			...
			foreach ($__vars['entries'] AS $__vars['cron']) {
				...
				if ($__vars['cron']['active']) {
					...
					if (((!$__vars['cron']['addon_id']) OR $__vars['cron']['AddOn']['active'])) {
						$__compilerTemp2 .= '
											' . (('Следующий запуск' . ': ') . $__templater->func('date_time', array($__vars['cron']['next_run'], ), true)) . '
										';
					} else {
						$__compilerTemp2 .= '
											' . 'Это задание планировщика связано с неактивным плагином. Оно не может быть выполнено, пока плагин не будет активирован.' . '
										';
					}
					$__compilerTemp2 .= '
									';
...
,array(
					...
					'submit' => 'true',
					'tooltip' => 'Включить/отключить \'' . $__vars['cron']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				));
				if (((!$__vars['cron']['addon_id']) OR $__vars['cron']['AddOn']['active'])) {
