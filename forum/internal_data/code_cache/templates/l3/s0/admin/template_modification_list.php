...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Модификации шаблонов');
	$__finalCompiled .= '
...
';
	...
	if ($__vars['groupedModifications']) {
			...
			foreach ($__vars['groupedModifications'] AS $__vars['addOnId'] => $__vars['modifications']) {
				...
				if ($__vars['addOnId']) {
					...
					$__compilerTemp3 .= '
						...
						'colspan' => '4',
						'_type' => 'cell',
						'html' => 'Пользовательские модификации',
					))) . '
							';
				}
				$__compilerTemp3 .= '
				...
				if ($__templater->isTraversable($__vars['modifications'])) {
					foreach ($__vars['modifications'] AS $__vars['modification']) {
						$__compilerTemp3 .= '
							...
							'submit' => 'true',
							'tooltip' => 'Включить/отключить \'' . $__vars['modification']['modification_key'] . '\'',
							'_type' => 'toggle',
							'html' => '',
						),
						array(
		...
		$__compilerTemp2 .= '
			<div class="block-body block-row">' . 'Пока нет ни одной модификации шаблонов.' . '</div>
		';
	}
