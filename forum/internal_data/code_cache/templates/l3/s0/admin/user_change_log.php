	...
	if ($__templater->isTraversable($__vars['changesGrouped'])) {
		...
		<tbody class="dataList-rowGroup">
			...
			if ($__vars['group']['editUser'] AND ($__vars['group']['editUser']['user_id'] != $__vars['group']['content_id'])) {
				$__compilerTemp2 .= '
							<li>' . 'Изменил(а) ' . $__templater->escape($__vars['group']['editUser']['username']) . '' . '</li>
						';
			}
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Журнал изменений пользователей' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['user']['username']));
	$__finalCompiled .= '
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Пока не записано никаких изменений.' . '</div>
';
	}
	$__finalCompiled .= '
