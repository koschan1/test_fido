...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Рекламные позиции');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить рекламную позицию', array(
		...
		if ($__templater->isTraversable($__vars['advertisingPositions'])) {
			foreach ($__vars['advertisingPositions'] AS $__vars['advertisingPosition']) {
				$__compilerTemp1 .= '
					...
					'submit' => 'true',
					'tooltip' => 'Включить/отключить \'' . $__vars['advertisingPosition']['position_id'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
