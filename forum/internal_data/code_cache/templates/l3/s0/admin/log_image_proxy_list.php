...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Журнал проксирования изображений');
	$__finalCompiled .= '
...
' . $__templater->form('
	<div class="block-container">
		...
		'class' => 'input--inline',
			...
			<span>
				' . 'Сортировать по' . $__vars['xf']['language']['label_separator'] . '
		...
		if ($__templater->isTraversable($__vars['entries'])) {
			foreach ($__vars['entries'] AS $__vars['entry']) {
				...
				if ($__vars['entry']['file_size']) {
					$__compilerTemp3 .= '<li>' . 'Размер' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['entry']['file_size'], array(array('file_size', array()),), true) . '</li>';
				}
