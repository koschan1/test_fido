...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['prefix'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Добавить префикс');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Редактировать префикс' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['prefix']['title']));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
';
	...
	if ($__templater->method($__vars['prefix'], 'hasRelation', array('MasterDescription', ))) {
		$__compilerTemp1 .= '
			...
			'label' => 'Описание',
			'explain' => 'Если этот префикс требует дополнительного объяснения его использования, Вы можете ввести описание. Можно использовать HTML.',
		)) . '
			';
	}
	$__compilerTemp2 = '';
	if ($__templater->method($__vars['prefix'], 'hasRelation', array('MasterUsageHelp', ))) {
		$__compilerTemp2 .= '
			...
			'label' => 'Справка по использованию префикса',
			'explain' => 'Если Вы хотите объяснить назначение этого префикса пользователям, введите описание здесь. Можно использовать HTML.',
		)) . '
			';
	}
