...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['profile'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Добавить платёжный профиль' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['provider']['title']));
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Редактировать платёжный профиль' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['provider']['title']) . ' - ' . $__templater->escape($__vars['profile']['title']));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
' . $__templater->form('
	<div class="block-container">
		...
		'explain' => 'Введите отображаемое название для этого платёжного профиля, оно будет показано пользователям при покупке. Если отображаемое название не указано, то вместо него будет использоваться заголовок.',
	)) . '
