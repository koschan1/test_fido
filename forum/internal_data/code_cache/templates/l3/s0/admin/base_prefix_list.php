...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<div class="buttonGroup">
		' . $__templater->button('Добавить префикс', array(
		'href' => $__templater->func('link', array($__vars['linkPrefix'] . '/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
		' . $__templater->button('Добавить группу префиксов', array(
	...
	';
			...
			foreach ($__vars['prefixGroups'] AS $__vars['prefixGroupId'] => $__vars['prefixGroup']) {
				...
				if (($__templater->func('count', array($__vars['prefixGroups'], ), false) > 1)) {
					...
					if (($__vars['prefixGroupId'] > 0)) {
						...
						$__compilerTemp2[] = array(
							'_type' => 'cell',
							'html' => $__vars['xf']['language']['parenthesis_open'] . 'Без группы' . $__vars['xf']['language']['parenthesis_close'],
						);
						$__compilerTemp2[] = array(
							'_type' => 'cell',
							'html' => '&nbsp;',
				...
				if ($__compilerTemp4) {
					...
					if (($__templater->func('count', array($__vars['prefixesGrouped'][$__vars['prefixGroupId']], ), false) OR ($__vars['prefixGroupId'] > 0))) {
						...
						if (($__templater->func('count', array($__vars['prefixGroups'], ), false) > 1)) {
							$__compilerTemp5 .= '
											' . 'В эту группу пока не добавлено ни одного префикса.' . '
										';
						} else {
							$__compilerTemp5 .= '
											' . 'Пока не добавлено ни одного префикса.' . '
										';
						}
						$__compilerTemp1 .= $__templater->dataRow(array(
		...
		<div class="block-container">
			...
			'_type' => 'option',
				...
				<span class="block-footer-controls">' . $__templater->button('Быстрая установка', array(
			'type' => 'submit',
			'name' => 'quickset',
			'value' => '1',
		), '', array(
