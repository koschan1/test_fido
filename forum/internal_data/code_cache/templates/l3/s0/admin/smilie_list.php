...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Смайлы');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	<div class="buttonGroup">
		' . $__templater->button('Добавить смайл', array(
		'href' => $__templater->func('link', array('smilies/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
		' . $__templater->button('Добавить категорию', array(
		...
		'title' => 'Дополнительные параметры',
			...
			<div class="menu" data-menu="menu" aria-hidden="true">
				<div class="menu-content">
					...
					<a href="' . $__templater->func('link', array('smilies/sort', ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Сортировать' . '</a>
					<a href="' . $__templater->func('link', array('smilies/import', ), true) . '" class="menu-linkRow">' . 'Импорт' . '</a>
					<a href="' . $__templater->func('link', array('smilies', array(), array('export' => 1, ), ), true) . '" class="menu-linkRow">' . 'Экспорт' . '</a>
				</div>
			</div>
		</div>
	</div>
...
';
	...
	if ($__templater->isTraversable($__vars['smilieData']['smilieCategories'])) {
		foreach ($__vars['smilieData']['smilieCategories'] AS $__vars['smilieCategoryId'] => $__vars['smilieCategory']) {
			...
			if ($__compilerTemp3) {
				...
				if (($__templater->func('count', array($__vars['smilieData']['smilieCategories'], ), false) > 1)) {
					$__compilerTemp6 .= '
									' . 'В эту категорию пока не добавлено ни одного смайла.' . '
								';
				} else {
					$__compilerTemp6 .= '
									' . 'Пока не добавлено ни одного смайла.' . '
								';
				}
				$__compilerTemp1 .= $__templater->dataRow(array(
	...
	<div class="block-container">
		...
		<h2 class="block-tabHeader tabs" role="tablist">
			' . '
			<a class="tabs-tab ' . ($__vars['exportView'] ? '' : 'is-active') . '" role="tab" tabindex="0" aria-controls="smilie-list" href="' . $__templater->func('link', array('smilies', ), true) . '">' . 'Смайлы' . '</a>
			<a class="tabs-tab ' . ($__vars['exportView'] ? 'is-active' : '') . '" role="tab" tabindex="0" aria-controls="export-mode" href="' . $__templater->func('link', array('smilies', array(), array('export' => 1, ), ), true) . '">' . 'Экспорт' . '</a>
			' . '
		</h2>
