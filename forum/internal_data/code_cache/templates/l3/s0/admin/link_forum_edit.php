...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['node'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Создать раздел-ссылку');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Редактировать раздел-ссылку' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['node']['title']));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
' . $__templater->form('
	<div class="block-container">
		...
		'label' => 'Ссылка',
		'explain' => 'При переходе в этот раздел, пользователи будут перенаправлены по указанной ссылке.',
	)) . '
