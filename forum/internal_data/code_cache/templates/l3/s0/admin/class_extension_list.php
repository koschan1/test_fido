...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Расширения классов');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить расширение класса', array(
...
';
	...
	';
		...
		if ($__templater->isTraversable($__vars['extensions'])) {
			foreach ($__vars['extensions'] AS $__vars['addOnId'] => $__vars['addOnExtensions']) {
				...
				if ($__vars['addOns'][$__vars['addOnId']]) {
					...
					$__compilerTemp2 .= '
										' . 'Пользовательские расширения классов' . '
									';
				}
				...
				if ($__templater->isTraversable($__vars['addOnExtensions'])) {
					foreach ($__vars['addOnExtensions'] AS $__vars['extension']) {
						...
						array(
							...
							'submit' => 'true',
							'tooltip' => 'Включить/отключить \'' . $__vars['extension']['from_class'] . '\'',
							'_type' => 'toggle',
							'html' => '',
						),
						array(
		...
		<div class="block-container">
			...
			'_type' => 'cell',
			'html' => 'Имя базового класса',
		),
		array(
			'_type' => 'cell',
			'html' => 'Имя класса расширения',
		),
		array(
			'_type' => 'cell',
			'html' => '&nbsp;',
