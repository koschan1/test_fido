...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Виджеты');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить виджет', array(
				...
				if (!$__templater->test($__vars['groupedWidgets'][$__vars['positionId']], 'empty', array())) {
					...
					if ($__vars['positionId'] == '') {
						$__compilerTemp1 .= '
							...
							'label' => $__vars['xf']['language']['parenthesis_open'] . 'Нет позиции' . $__vars['xf']['language']['parenthesis_close'],
							'explain' => $__templater->filter('Виджеты, позиция которых не определена не будут нигде отображены, но Вы можете вызвать настраиваемый виджет непосредственно в нужных шаблонах, используя синтаксис: <code>&lt;xf:widget key="widget_key" /&gt;</code>.', array(array('raw', array()),), true),
							'colspan' => '2',
						), array()) . '
								';
					} else {
					...
					if ($__templater->isTraversable($__vars['groupedWidgets'][$__vars['positionId']])) {
						foreach ($__vars['groupedWidgets'][$__vars['positionId']] AS $__vars['widget']) {
							$__compilerTemp1 .= '
								...
								'tooltip' => ($__vars['positionId'] ? 'Удалить из этой позиции' : 'Удалить'),
								'_type' => 'delete',
								'html' => '',
							))) . '
								';
