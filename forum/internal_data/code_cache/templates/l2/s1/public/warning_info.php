...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	' . $__templater->form('
		...
		'value' => 'now',
		'label' => 'Истекает сейчас',
		'_type' => 'option',
	),
	array(
		'value' => 'future',
		'label' => 'Истекает через' . $__vars['xf']['language']['label_separator'],
		'_dependent' => array('
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Предупреждение ' . $__templater->escape($__vars['user']['username']) . '');
	$__finalCompiled .= '
...
<div class="form block">
	...
	if ($__vars['warning']['is_expired']) {
		$__compilerTemp3 .= '
					<span class="u-muted">' . $__vars['xf']['language']['parenthesis_open'] . 'Истекло' . $__vars['xf']['language']['parenthesis_close'] . '</span>
				';
	} else if ($__vars['warning']['expiry_date']) {
		$__compilerTemp3 .= '
					<span class="u-muted">' . $__vars['xf']['language']['parenthesis_open'] . 'Истекает ' . $__templater->func('date', array($__vars['warning']['expiry_date'], ), true) . '' . $__vars['xf']['language']['parenthesis_close'] . '</span>
				';
	}
	$__finalCompiled .= $__templater->formRow('
	...
	if ($__templater->method($__vars['warning'], 'canDelete', array()) AND $__templater->method($__vars['warning'], 'canEditExpiry', array())) {
		$__finalCompiled .= '
			<h2 class="block-tabHeader tabs" data-xf-init="tabs" role="tablist">
				<a class="tabs-tab is-active" role="tab" tabindex="0" aria-controls="' . $__templater->func('unique_id', array('warningDelete', ), true) . '">' . 'Удалить предупреждение' . '</a>
				<a class="tabs-tab is-active" role="tab" tabindex="0" aria-controls="' . $__templater->func('unique_id', array('warningUpdate', ), true) . '">' . 'Обновить предупреждение' . '</a>
			</h2>
			<ul class="tabPanes">
