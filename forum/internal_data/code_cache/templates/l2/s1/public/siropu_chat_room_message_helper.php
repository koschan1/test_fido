...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if (($__vars['xf']['options']['siropuChatUserTagging'] == 'button') AND (!$__templater->method($__vars['message'], 'isSelf', array()))) {
		$__compilerTemp1 .= '
			<a role="button" class="siropuChatTag" title="' . $__templater->filter('Ник в ответ', array(array('for_attr', array()),), true) . '">@</a>
		';
	}
	$__compilerTemp2 = '';
	if ($__vars['message']['User']) {
		...
		if (($__vars['xf']['options']['siropuChatUserTagging'] == 'username') AND (!$__templater->method($__vars['message'], 'isSelf', array()))) {
			$__compilerTemp2 .= '
				...
				'notooltip' => 'true',
				'title' => $__templater->filter('Ник в ответ', array(array('for_attr', array()),), false),
			))) . '
			';
		} else {
			$__compilerTemp2 .= '
	...
	if ($__templater->method($__vars['message'], 'isEdited', array()) AND $__templater->method($__vars['message'], 'canViewHistory', array())) {
		$__finalCompiled .= '
		<a href="' . $__templater->func('link', array('chat/message/history', $__vars['message'], ), true) . '" class="siropuChatMessageEdited" title="' . $__templater->filter('Посмотреть историю', array(array('for_attr', array()),), true) . '" data-xf-init="tooltip" data-xf-click="overlay" data-cache="false">' . 'Отредактировано' . '</a>
	';
	}
	$__finalCompiled .= '
';
