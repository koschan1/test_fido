...
';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Введите детали платежа');
	$__finalCompiled .= '
...
<div class="blocks">
	' . $__templater->form('
		...
		'id' => 'brand-icon',
					...
					<div class="formRow-explain">' . 'Платежи безопасно обрабатываются сервисом <a href="' . 'https://braintreepayments.com/' . '" target="_blank">' . 'Braintree' . '</a>. Мы не обрабатываем и не храним Ваши платёжные данные.' . '</div>
				', array(
		...
		'rowtype' => 'input',
				...
				' . $__templater->formRow('
					' . $__templater->button('
						' . 'Оплатить ' . $__templater->filter($__vars['purchase']['cost'], array(array('currency', array($__vars['purchase']['currency'], )),), true) . '' . '
					', array(
		'type' => 'submit',
		'icon' => 'payment',
	), '', array(
		...
		if ($__vars['paymentProfile']['options']['apple_pay_enable']) {
			...
			' . $__templater->form('
				...
				'class' => 'button--apple js-applePayButton',
						...
						', array(
				'rowtype' => 'button',
				'label' => 'Оплатить с помощью Apple Pay',
			)) . '
					</div>
				</div>
			', array(
			...
			' . $__templater->form('
				...
				<div class="block-container">
					<div class="block-body">
						...
						', array(
				'rowtype' => 'button',
				'label' => 'Оплатить с помощью PayPal',
			)) . '
					</div>
				</div>
			', array(
