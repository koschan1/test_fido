...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Наказание в комнате');
	$__finalCompiled .= '
...
<div class="block">
	...
	if ($__templater->func('count', array($__vars['sanctions'], ), false)) {
			...
			foreach ($__vars['sanctions'] AS $__vars['sanction']) {
				...
				$__vars['extraData'] = $__templater->preEscaped('
							<div style="text-align: right;">
								<b>' . 'Тип наказания' . ':</b> ' . $__templater->escape($__templater->method($__vars['sanction'], 'getTypePhrase', array())) . '<br>
								<b>' . 'Наказание началось' . ':</b> ' . $__templater->func('date_dynamic', array($__vars['sanction']['sanction_start'], array(
				))) . '<br>
								<b>' . 'Наказание заканчивается' . ':</b>
								' . $__compilerTemp1 . '
								<br><b>' . 'Наказание за' . ':</b> ' . $__templater->func('username_link', array($__vars['sanction']['Author'], false, array(
				))) . '
								' . $__compilerTemp2 . '
								' . $__compilerTemp3 . '
							</div>
		...
		$__finalCompiled .= '
				<li class="block-row">' . 'В этой комнате нет пользователей, к которым были применены наказания.' . '</li>
			';
	}
	$__finalCompiled .= '
		</ol>
