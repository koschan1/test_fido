	...
	if ($__vars['shared']) {
				...
				if ($__templater->isTraversable($__vars['share']['ips'])) {
					foreach ($__vars['share']['ips'] AS $__vars['ip']) {
						...
						if ($__vars['ip']['total'] > 1) {
							$__finalCompiled .= '
															<li>' . '' . $__templater->filter($__vars['ip']['total'], array(array('number', array()),), true) . ' раз(а)' . '</li>
		...
		$__finalCompiled .= '
				...
				<div class="block-footer">
					<span class="block-footer-counter">' . 'Найдено пользователей: ' . $__templater->func('count', array($__vars['shared'], ), true) . '.' . '</span>
				</div>
			';
	} else {
		$__finalCompiled .= '
				<div class="block-body block-row">' . 'Соответствующие пользователи не найдены.' . '</div>
			';
	}
	$__finalCompiled .= '
		</div>
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Общие IP-адреса ' . $__templater->escape($__vars['user']['username']) . '');
	$__finalCompiled .= '
