...
';
	...
	if ($__vars['positionId']) {
		$__compilerTemp1 .= '
					' . 'Пожалуйста, подтвердите удаление следующего виджета из позиции \'' . $__templater->escape($__vars['positionId']) . '\'.' . '
				';
	} else {
		$__compilerTemp1 .= '
	...
	if ($__templater->func('count', array($__vars['widget']['positions'], ), false) == 1) {
		$__compilerTemp2 .= '
			...
			'name' => 'delete',
			'label' => 'Это единственная позиция для этого виджета. Окончательное удаление виджета.',
			'_type' => 'option',
		)), array(
		...
		$__compilerTemp2 .= '
			<div class="block-row">
				<div class="blockMessage blockMessage--close blockMessage--warning blockMessage--iconic blockMessage--iconic">
					' . 'Этот виджет в настоящее время используется в позиции(ях): ' . $__templater->func('count', array($__vars['widget']['positions'], ), true) . '' . '
				</div>
			</div>
		';
	}
