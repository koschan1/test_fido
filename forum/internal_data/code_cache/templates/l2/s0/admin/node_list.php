...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Узлы');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	<div class="buttonGroup">
		' . $__templater->button('Добавить узел', array(
...
,array(
				...
				if ($__vars['moderators'][$__vars['node']['node_id']]) {
					...
					$__compilerTemp4[] = array(
						...
						'label' => 'Модераторы (' . $__templater->func('count', array($__vars['moderators'][$__vars['node']['node_id']], ), false) . ')',
						'_type' => 'popup',
						'html' => '

									<div class="menu" data-menu="menu" aria-hidden="true">
										<div class="menu-content">
											<h3 class="menu-header">' . 'Модераторы' . '</h3>
											' . $__compilerTemp5 . '
											<hr class="menu-separator" />
											<a href="' . $__templater->func('link', array('moderators/add', null, array('type' => 'node', 'type_id' => array('node' => $__vars['node']['node_id'], ), ), ), true) . '" class="menu-linkRow">' . 'Добавить модератора' . '</a>
										</div>
									</div>
								',
					);
					...
					$__compilerTemp4[] = array(
						...
						'class' => 'u-hideMedium',
						'_type' => 'action',
						'html' => 'Добавить модератора',
					);
				}
				$__compilerTemp4[] = array(
					'class' => 'dataList-cell--action u-hideMedium',
					'label' => 'Добавить' . $__vars['xf']['language']['ellipsis'],
					'_type' => 'popup',
					'html' => '

								<div class="menu" data-menu="menu" aria-hidden="true">
									<div class="menu-content">
										<h3 class="menu-header">' . 'Добавить' . $__vars['xf']['language']['ellipsis'] . '</h3>
										<a href="' . $__templater->func('link', array('nodes/add', null, array('parent_node_id' => $__vars['node']['parent_node_id'], ), ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Одноуровневый элемент' . '</a>
										<a href="' . $__templater->func('link', array('nodes/add', null, array('parent_node_id' => $__vars['node']['node_id'], ), ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Дочерний элемент' . '</a>
									</div>
								</div>
							',
				);
