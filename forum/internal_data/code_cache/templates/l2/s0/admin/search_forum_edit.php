...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['node'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Добавить раздел поиска');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Редактировать раздел поиска' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['node']['title']));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
' . $__templater->form('
	<div class="block-container">
		...
		<h2 class="block-formSectionHeader">
			<span class="block-formSectionHeader-aligner">' . 'Результаты поиска' . '</span>
		</h2>
		<div class="block-body">
			<hr class="formRowSep" />
	...
	array(
		...
		'label' => 'Длительность кэширования',
		'explain' => 'Результаты поиска, включая их порядок сортировки, будут кэшированны в течение этого времени. Меньшее значение даст большую точность, но снизит производительность.',
	)) . '
		</div>

		<h2 class="block-formSectionHeader">
			<span class="block-formSectionHeader-aligner">' . 'Критерии темы' . '</span>
		</h2>
		<div class="block-body">
