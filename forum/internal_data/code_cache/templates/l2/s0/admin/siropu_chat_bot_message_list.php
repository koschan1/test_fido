...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Сообщения Бота');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить сообщение Бота', array(
		...
		if ($__templater->isTraversable($__vars['messages'])) {
			foreach ($__vars['messages'] AS $__vars['message']) {
				...
				array(
					...
					'submit' => 'true',
					'tooltip' => 'Включить/Выключить сообщение Бота',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
