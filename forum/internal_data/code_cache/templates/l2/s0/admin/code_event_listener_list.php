...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Обработчики событий');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить обработчик события', array(
				...
				if ($__templater->isTraversable($__vars['listenerEntity'])) {
					foreach ($__vars['listenerEntity'] AS $__vars['listener']) {
						...
						array(
							...
							'submit' => 'true',
							'tooltip' => 'Включить/отключить \'' . $__vars['listener']['event_id'] . '\'',
							'_type' => 'toggle',
							'html' => '',
						),
						array(
