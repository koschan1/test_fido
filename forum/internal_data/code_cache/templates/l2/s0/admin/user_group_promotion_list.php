...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Повышения прав групп');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Управление пользователями с повышениями', array(
		...
		if ($__templater->isTraversable($__vars['userGroupPromotions'])) {
			foreach ($__vars['userGroupPromotions'] AS $__vars['userGroupPromotion']) {
				$__compilerTemp1 .= '
					...
					'submit' => 'true',
					'tooltip' => 'Включить/отключить \'' . $__vars['userGroupPromotion']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Пока не определено ни одного повышения.' . '</div>
';
	}
