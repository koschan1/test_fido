...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Объявления');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить объявление', array(
...
';
	...
	';
		...
		<div class="blockMessage blockMessage--important">
			' . 'Следующие уведомления могут содержать более не соответствующие критерии страницы' . $__vars['xf']['language']['label_separator'] . '
					...
					if ($__templater->isTraversable($__vars['notices'][$__vars['typeId']])) {
						foreach ($__vars['notices'][$__vars['typeId']] AS $__vars['notice']) {
							$__compilerTemp1 .= '
								...
								'submit' => 'true',
								'tooltip' => 'Включить/отключить \'' . $__vars['notice']['title'] . '\'',
								'_type' => 'toggle',
								'html' => '',
							))) . '
							';
