...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['depth'] == 1) {
		...
		if ($__vars['navigation']['navigation_id'] == $__vars['xf']['app']['defaultNavigationId']) {
			...
			$__compilerTemp1[] = array(
				...
				'submit' => 'true',
				'tooltip' => 'Включить/отключить \'' . $__vars['navigation']['title'] . '\'',
				'_type' => 'toggle',
				'html' => '',
			);
			$__compilerTemp1[] = array(
	...
	if (($__vars['depth'] == 1) AND $__templater->test($__vars['children'], 'empty', array())) {
		...
		' . $__templater->dataRow(array(
			...
			'colspan' => '3',
			'_type' => 'cell',
			'html' => 'Этот раздел в настоящее время пуст.',
		))) . '
	';
	} else {
		$__finalCompiled .= '
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Публичная навигация');
	$__finalCompiled .= '
