...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Сортировка смайлов');
	$__finalCompiled .= '
	...
	if ($__templater->isTraversable($__vars['smilieData']['smilieCategories'])) {
		foreach ($__vars['smilieData']['smilieCategories'] AS $__vars['smilieCategoryId'] => $__vars['smilieCategory']) {
			...
			';
				...
				$__compilerTemp1 .= '
				<h4 class="block-minorHeader">' . 'Смайлы без категории' . '</h4>
			';
			}
			$__compilerTemp1 .= '
				...
				foreach ($__vars['smilieData']['smilies'][$__vars['smilieCategoryId']] AS $__vars['smilieId'] => $__vars['smilie']) {
					...
					$__compilerTemp2 .= '
								<li class="nestable-item" data-id="' . $__templater->escape($__vars['smilieId']) . '">
									<div class="nestable-handle nestable-handle--full" aria-label="' . $__templater->filter('Перетаскивание', array(array('for_attr', array()),), true) . '">' . $__templater->fontAwesome('fa-bars', array(
					)) . '</div>
