...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	';
		...
		$__templater->modifySideNavHtml(null, '
			<div class="block">
				<div class="block-container">
					<h3 class="block-header">' . 'Список тем' . '</h3>
		...
		$__finalCompiled .= '
		';
		$__templater->setPageParam('sideNavTitle', 'Список тем');
		$__finalCompiled .= '
	';
	}
	$__finalCompiled .= '
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<a class="' . $__templater->escape($__vars['baseClass']) . ' ' . ((($__vars['pageSelected'] == 'started') AND ($__vars['xf']['visitor']['user_id'] == $__vars['user']['user_id'])) ? $__templater->escape($__vars['selectedClass']) : '') . '"
		href="' . $__templater->func('link', array('find-threads/started', ), true) . '" rel="nofollow">' . 'Ваши темы' . '</a>
	<a class="' . $__templater->escape($__vars['baseClass']) . ' ' . (($__vars['pageSelected'] == 'contributed') ? $__templater->escape($__vars['selectedClass']) : '') . '"
		href="' . $__templater->func('link', array('find-threads/contributed', ), true) . '" rel="nofollow">' . 'Темы с Вашими сообщениями' . '</a>
	<a class="' . $__templater->escape($__vars['baseClass']) . ' ' . (($__vars['pageSelected'] == 'unanswered') ? $__templater->escape($__vars['selectedClass']) : '') . '"
		href="' . $__templater->func('link', array('find-threads/unanswered', ), true) . '" rel="nofollow">' . 'Темы без ответов' . '</a>
	<a class="' . $__templater->escape($__vars['baseClass']) . ' ' . (($__vars['pageSelected'] == 'watched') ? $__templater->escape($__vars['selectedClass']) : '') . '"
		href="' . $__templater->func('link', array('watched/threads', ), true) . '" rel="nofollow">' . 'Отслеживаемые темы' . '</a>
';
	return $__finalCompiled;
}
)),
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['pageSelected'] == 'started') {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Темы, созданные ' . $__templater->escape($__vars['user']['username']) . '');
...
';
		...
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Темы с сообщениями от ' . $__templater->escape($__vars['user']['username']) . '');
		$__templater->pageParams['pageNumber'] = $__vars['page'];
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Темы без ответов');
		$__templater->pageParams['pageNumber'] = $__vars['page'];
		$__finalCompiled .= '
	';
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Нет тем для отображения.' . '</div>
';
	}
	$__finalCompiled .= '
