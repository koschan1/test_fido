...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Закладки');
	$__finalCompiled .= '
...
<div class="block">
	...
	if ($__vars['label']) {
		$__compilerTemp1 .= '
								<li><a href="' . $__templater->func('link', array('account/bookmarks', ), true) . '"
									...
									<span class="filterBar-filterToggle-label">' . 'Ярлык' . $__vars['xf']['language']['label_separator'] . '</span>
	...
	$__finalCompiled .= '
				...
				<div class="menu menu--wide" data-menu="menu" aria-hidden="true">
					<div class="menu-content">
						<h4 class="menu-header">' . 'Фильтр по тегам' . $__vars['xf']['language']['label_separator'] . '</h4>
						' . $__templater->form('
	...
	if (!$__templater->test($__vars['bookmarks'], 'empty', array())) {
		...
		if ($__vars['label']) {
			$__finalCompiled .= '
					<div class="block-row">' . 'Нет элементов, соответствующих Вашему фильтру.' . '</div>
				';
		} else {
			$__finalCompiled .= '
					<div class="block-row">' . 'Вы ещё не добавили ни одной закладки.' . '</div>
				';
		}
		$__finalCompiled .= '
			';
