...
<div class="block"' . $__templater->func('widget_data', array($__vars['widget'], ), true) . '>
	<div class="block-container">
		<h3 class="block-minorHeader">' . ($__templater->escape($__vars['title']) ?: 'Комнаты в чате') . '</h3>
	...
	if ($__vars['options']['search'] AND ($__templater->func('count', array($__vars['rooms'], ), false) >= $__vars['options']['search'])) {
		$__finalCompiled .= '
				<div class="block-row">
					<input type="search" class="input siropuChatFindItem" autocomplete="off" placeholder="' . 'Поиск в комнате...' . '" data-xf-init="siropu-chat-find-item">
				</div>
			';
	}
	$__finalCompiled .= '
		...
		foreach ($__vars['rooms'] AS $__vars['room']) {
			...
			if ($__vars['xf']['visitor']['user_id']) {
				...
				if ($__templater->method($__vars['room'], 'canJoin', array(null, false, )) AND (!$__templater->method($__vars['room'], 'isJoined', array()))) {
					$__finalCompiled .= '
								<a href="' . $__templater->func('link', array('chat/room', $__vars['room'], ), true) . '" class="button" title="' . $__templater->filter('Присоединиться к комнате', array(array('for_attr', array()),), true) . '" data-action="join">' . $__templater->fontAwesome('fas fa-sign-in', array(
					)) . '</a>
							';
				}
				$__finalCompiled .= '
				...
				if ($__templater->method($__vars['room'], 'isJoined', array()) AND $__templater->method($__vars['room'], 'canLeave', array())) {
					$__finalCompiled .= '
								<a href="' . $__templater->func('link', array('chat/room/leave', $__vars['room'], ), true) . '" class="button button--cta" title="' . $__templater->filter('Покинуть комнату', array(array('for_attr', array()),), true) . '" data-action="leave">' . $__templater->fontAwesome('fas fa-sign-out', array(
					)) . '</a>
							';
				}
				$__finalCompiled .= '
