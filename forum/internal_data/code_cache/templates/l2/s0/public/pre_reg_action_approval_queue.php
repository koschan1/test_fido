...
'extensions' => array('header' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['details']['title'] AND $__vars['details']['title_link']) {
		$__finalCompiled .= '
			' . 'Действия перед регистрацией' . $__vars['xf']['language']['label_separator'] . ' <a href="' . $__templater->escape($__vars['details']['title_link']) . '">' . $__templater->escape($__vars['details']['title']) . '</a>
		';
	} else if ($__vars['details']['title']) {
		$__finalCompiled .= '
			' . 'Действия перед регистрацией' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['details']['title']) . '
		';
	} else {
		$__finalCompiled .= '
