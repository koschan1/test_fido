                       ...
                       <div class="init-slider-tariff" >
                           ...
                           foreach ($data['tariffs'] as $value) {
                               ...
                               ?>
                                   ...
                                   <div class="user-tariff-box-item <?php if($_SESSION['profile']['data']['clients_tariff_id'] == $value['services_tariffs_id']){ echo 'user-tariff-box-item-gray active'; } ?>" data-id="<?php echo $value['services_tariffs_id']; ?>" >
                                       <h5><?php echo $value['services_tariffs_name']; ?></h5>
                                       <?php if($value['services_tariffs_price']){ ?>

                                       <p><strong><?php echo $Main->outPrices( array("new_price"=> array("price"=>$value["services_tariffs_new_price"], "tpl"=>'<span class="tariff_newPrice" >{price}</span>'), "price"=>array("price"=>$value["services_tariffs_price"], "tpl"=>'<span class="tariff_oldPrice" >{price}</span>') ) ); ?></strong> <span>за <?php echo $value['services_tariffs_days']; ?> <?php echo ending($value['services_tariffs_days'],$ULang->t('день'),$ULang->t('дня'),$ULang->t('дней')); ?></span></p>
                                       <?php }else{ ?>
                                       <p><strong><?php echo $ULang->t("Бесплатно"); ?></strong> <?php if($value['services_tariffs_days']){ ?><span><?php echo $ULang->t("на"); ?> <?php echo $value['services_tariffs_days']; ?> <?php echo ending($value['services_tariffs_days'],$ULang->t('день'),$ULang->t('дня'),$ULang->t('дней')); ?></span><?php } ?></p>
                                       <?php } ?>
                                       <p><?php echo $value['services_tariffs_desc']; ?></p>
                                       ...
                                       <ul>
                                           ...
                                           if(count($services)){
                                               foreach ($services as $service_id) {
                                                   ...
                                                   ?>
                                                   <li><span class="user-tariff-box-item-service-icon" ><i class="las la-check"></i></span><span class="user-tariff-box-item-service-name" ><?php echo $checklist['services_tariffs_checklist_name']; ?>
                                                   <?php if($checklist['services_tariffs_checklist_desc']){ ?>
                                                   <i data-tippy-placement="top" title="<?php echo $checklist['services_tariffs_checklist_desc']; ?>" class="las la-info-circle"></i>
                                                   <?php } ?>
                                                   </span></li>
                                                   <?php
                                               }
